...
def function(omega, theta, a=0, b=500, h=2e-1):
    omegaPoints = []
    thetaPoints = []
    snapOmega = []
    snapTheta = []
    ...
    for t in tPoints:
        ...
        if (np.abs( w_d*t/(2*np.pi) - np.floor(w_d*t/(2*np.pi))) <= h/100):
...
theta1, omega1, time, snapTheta, snapOmega = function(0.0, 0.2)
theta2, omega2, time, dum1, dum2 = function(0.0, 0.2+1e-3)
