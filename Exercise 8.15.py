...
# TODO: Use 4th-Order RK method to solve the equations of motion for L = 40cm,
...
def doublePendulum(th1, th2, w1, w2):
    ...
    for t in tPts:
        ...
        elif (th1 > np.pi): th1 -= 2*np.pi
        if (th2 < -np.pi): th2 += 2*np.pi
        elif (th2 > np.pi): th2 -= 2*np.pi
    
    return th1Pts, th2Pts, w1Pts, w2Pts
